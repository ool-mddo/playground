...
def find_benchmark_index(network_index_data, network, benchmark_topology):
    return next(
        (
            n
            for n in network_index_data
            if n["network"] == network and n["snapshot"] == benchmark_topology
        ),
        None,
    )
...
def candidates_index_items(original_candidate_data):
    return [
        netoviz_index_item(c["network"], c["snapshot"]) for c in original_candidate_data
    ]


def rev_candidates_index_items(original_candidate_data):
    return [
        netoviz_index_item(c["network"], reverse_ns_topology_name(c["snapshot"]))
        for c in original_candidate_data
    ]
...
def append_netoviz_index_item(netoviz_index, index_item):
    item = next(
        (
            i
            for i in netoviz_index
            if i["network"] == index_item["network"]
            and i["snapshot"] == index_item["snapshot"]
        ),
        None,
    )
    if item:
...
def append_netoviz_index(
    netoviz_index, network_index_data, network, benchmark_topology
):
    benchmark_index_data = find_benchmark_index(
        network_index_data, network, benchmark_topology
    )
    if benchmark_index_data:
        append_netoviz_index_item(netoviz_index, benchmark_index_data)
    else:
        append_netoviz_index_item(
            netoviz_index, netoviz_index_item(network, benchmark_topology)
        )
...
def main():
    ...
    parser.add_argument(
        "-s", "--step", type=int, required=True, help="Step number in a phase"
    )
    parser.add_argument(
        "-i",
        "--network_index",
        type=str,
        default=os.environ.get("NETWORK_INDEX"),
        help="Network index file (json)",
    )
    parser.add_argument(
        "-d",
        "--session_dir",
        type=str,
        default=os.environ.get("USECASE_SESSION_DIR"),
        help="Usecase session dir",
    )
    args = parser.parse_args()

    if args.network_index is None or args.session_dir is None:
        print(
            f"network_index(={args.network_index}) and/or session_dir(={args.session_dir}) are not specified",
            file=sys.stderr,
        )
        parser.print_help()
        sys.exit(1)
    ...
    for phase in range(1, args.phase + 1):
        original_candidate_data = open_json(
            f"{args.session_dir}/original_candidate_list_{phase}.json"
        )
        ...
        if step_count(phase, 1) <= step_count_end:
            ...
            append_netoviz_index(
                netoviz_index, network_index_data, args.network, benchmark_topology
            )
        ...
        if step_count(phase, 2) <= step_count_end:
            ...
            append_netoviz_index(
                netoviz_index, network_index_data, args.network, rev_benchmark_topology
            )
