...
if (ARGC != 1) {
    print "Usage: gnuplot region_data_single.gp <date_dir>"
    exit
}
DATA_DIR = ARG1
...
set output "region_data_single_" . DATA_DIR . ".eps"

set title "Time required for deployment and memory used"
...
plot \
    ...
    DATA_FILE using 2:3 with points ls 2 linewidth 4 title "Time required for deployment" axis x1y2 noenhanced, \
    g(x) with lines ls 2 linewidth 2 title "Fit: Time required for deployment" axis x1y2