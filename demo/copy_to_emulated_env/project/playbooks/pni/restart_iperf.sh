...
if "${WITH_CLAB:-true}"; then
  ansible-runner run . -p "/data/project/playbooks/${USECASE_COMMON_NAME}/restart_iperf.yaml" --container-option="--net=${API_BRIDGE}" \
    ...
    --cmdline "-e ansible_runner_dir=${ANSIBLE_RUNNER_DIR} -e playground_dir=${PLAYGROUND_DIR} -e login_user=${LOCALSERVER_USER} -e network_name=${NETWORK_NAME} -e usecase_name=${USECASE_NAME} -e usecase_common_name=${USECASE_COMMON_NAME} -e clab_restart=true -k -K "
fi
